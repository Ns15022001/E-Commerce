                ...
                orders?.map((order) => (
                  <Stack key={order._id} p={is480 ? 0 : 2} component={is480 ? "" : Paper} elevation={1} rowGap={2}>
                    ...
                    <Stack rowGap={2}>
                      {
                        order?.item?.map((product, idx) => (
                          <Stack key={idx} mt={2} flexDirection={'row'} rowGap={is768 ? '2rem' : ''} columnGap={4} flexWrap={is768 ? "wrap" : "nowrap"}>
                            <Stack>
                              <img
                            ...
                            <Stack rowGap={1} width={'100%'}>
                              ...
                              <Stack mt={2} alignSelf={is480 ? "flex-start" : 'flex-end'} flexDirection={'row'} columnGap={2}>
                                <Button size='small' component={Link} to={`/product-details/${product?.product?._id}`} variant='outlined'>
                                  View Product
                                </Button>
                                {
                    ...
                    <Stack mt={2} flexDirection={'row'} justifyContent={'space-between'}>
                      <Typography mb={2}>Status: {order?.status || 'N/A'}</Typography>
                    </Stack>
                  </Stack>
                ))
              }
