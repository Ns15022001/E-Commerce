...
export const Wishlist = () => {
  ...
  useEffect(() => {
    if (wishlistItemUpdateStatus === 'fulfilled') toast.success("Wishlist item updated")
    else if (wishlistItemUpdateStatus === 'rejected') toast.error("Error updating wishlist item")
                  ...
                  <Grid container gap={1} justifyContent={'center'} alignContent={'center'}>
                    {
                      wishlistItems.map((item, index) => (
                        <Stack key={item?._id || index} component={is480 ? "" : Paper} elevation={1}>
                          <ProductCard
                            item
                          ...
                          <Stack paddingLeft={2} paddingRight={2} paddingBottom={2}>
...

                            {
                              editIndex === index ? (
                                ...
                                <Box>
                                  <Typography sx={{ wordWrap: "break-word", color: item?.note ? 'text.primary' : 'GrayText' }}>
...

                            {
                              cartItems?.some(cartItem => cartItem?.product?._id === item?.product?._id) ? (
                                <Button sx={{ mt: 4 }} size='small' variant='outlined' component={Link} to={'/cart'}>Already in cart</Button>
                              ) : (
                                <Button sx={{ mt: 4 }} size='small' onClick={() => handleAddToCart(item?.product?._id)} variant='outlined'>Add To Cart</Button>
                              )
                            }
                          </Stack>
                        </Stack>
